...
class TimeSeries:
    def __init__(self, collection: ee.ImageCollection, dependent: str, modes: int = 3) -> None:
    ...
    def __add_independent(self, independent: str | list[str]) -> None:
        ...
        elif isinstance(independent, list) and all(isinstance(i, str) for i in independent):
            self.independent.extend(independent)
        else:
            raise TypeError('Independent variable must be a string or a list of strings.')
